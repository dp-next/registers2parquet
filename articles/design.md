...
[`convert()`](https://dp-next.github.io/fastreg/reference/convert.md)
returns a metadata tibble with one row per written chunk. This can be
queried with dplyr directly or rendered into a Quarto log.

### Return value of `convert()`

[`convert()`](https://dp-next.github.io/fastreg/reference/convert.md)
returns a tibble with one row per written chunk:
...
copies both `_targets.R` and `conversion_log.qmd` into the current
working directory. The Quarto doc reads `chunk_info` via
[`targets::tar_read()`](https://docs.ropensci.org/targets/reference/tar_read.html)
and produces an HTML or PDF log for review. The default is PDF, but it
