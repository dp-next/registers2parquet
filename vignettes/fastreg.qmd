...
Many of fastreg's functions depend on the correct locations to the original
SAS files and the eventual Parquet files. This applies to both the conversion
and writing functions, as well as to the reading functions. Through `options()`
you can set these paths in our two settings: `fastreg.project_rawdata_dir` and
...
in your user-level `.Rprofile`. To add to the file, at the top, write out
...
Which opens up the `.Rprofile` file for the project. You can then add the same `options()`
to that file and save it. The next time you open the project, those options will be set.
Or, to set it for all of your R projects and sessions, you can set it globally
in your user-level `.Rprofile`:
