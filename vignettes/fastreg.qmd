...
larger-than-memory data) with a default of reading 1 million rows, extracts 4-digit years from filenames for
partitioning, and lowercases column names. See `?convert_file` for
more details.
...
`convert_register()` uses `convert_file()` internally to reads files in chunks (to be able to handle
larger-than-memory data), extracts 4-digit years from filenames for
partitioning, and lowercases column names. See `?convert_file` and `?convert_register` for
more details.
...
files in a `Grunddata/` folder that looks like below:
...
recursively). This directory can contain different registers,
rather than just one as is expected in `convert_register()`. The `output_dir` directory is where the Parquet
...
The pipeline will find all SAS files from `input_dir` and
...
The final function reads the converted Parquet register data into
R. This function reads the data into a [DuckDB](https://duckdb.org/) table,
