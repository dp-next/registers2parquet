...
To list what Parquet files or datasets are available, there's the
helpful `list_parquet_files()` and `list_parquet_datasets()` functions.
These look in the `fastreg.project_workdata_dir` and
`fastreg.project_rawdata_dir` directories (set with `options()`) for any
Parquet files following a specific pattern. See the function help docs
for more details.
