...
Since the data is lazy, you can filter before collecting. Only the
matching rows are loaded into memory when using `dplyr::collect()` after
filtering:
...
You can also transform columns before filtering. For example,
`foed_dato` is stored character in the simulated data (`"YYYYMMDD"`), so
you can change it to a date (using `strptime`) before filtering it:

```{r filter-date}
register |>
  dplyr::mutate(foed_dato = strptime(foed_dato, "%Y%m%d")) |>
  dplyr::filter(foed_dato > "2000-01-01") |>
  dplyr::collect()
```
