...
Parquet files following a specific pattern. See the reference
...
function. This function looks for a given name of a register in either the
`fastreg.project_workdata_dir` or `fastreg.project_rawdata_dir`
directories (set with `options()`) and reads it as a DuckDB table. You
can also use the more specific `read_parquet_partition()` or
`read_parquet_file()` functions to read from a specific directory or
file path.

<!-- TODO: Needs the updated simulation functions to work -->

```{r read-file}
#| eval: false
file <- fastreg::read_register("bef")
file
```

Or directly with `read_parquet_partition()` or `read_parquet_file()`:

```{r read-partition}
file <- fastreg::read_parquet_partition(output_file_dir)
file

# Or a single file
fs::dir_ls(output_file_dir, glob = "*.parquet", recurse = TRUE)[1] |>
