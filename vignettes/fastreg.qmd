...
function, which creates two files: a `_targets.R` pipeline and
a `conversion-log.qmd` Quarto document. When running the
pipeline, the conversion log is written to a PDF, named
