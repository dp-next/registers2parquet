...
fastreg eases the converts SAS register files (`.sas7bdat`) into
...
subdirectories), then use `convert_to_parquet()` to convert them.
`convert_to_parquet()` expects files to be from the **same register**
based on file names.
...
withr::with_dir(fs::path_dir(output_dir), {
  fs::dir_tree(fs::path_file(output_dir))
})
```

`convert_to_parquet()` reads files in chunks (to be able to handle
larger-than-memory data), extracts 4-digit years from filenames for
partitioning, and lowercases column names. See `?convert_to_parquet` for
more details.
...
modified_content <- template_content |>
  stringr::str_replace("/path/to/register/sas/files/directory", config$input_path) |>
  stringr::str_replace("/path/to/output/directory",config$output_path)
...
```{r output-pipeline}
#| echo: false
withr::with_dir(fs::path_dir(config$output_path), {
  fs::dir_tree(fs::path_file(config$output_path))
})
```
