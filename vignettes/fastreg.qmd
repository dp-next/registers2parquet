...
registers_tbl <- fastreg::simulate_registers_nested_tbl(
  c("bef", "lmdb"),
  c("", "1999", "1999_1", "2020", "2021"),
  n = 1000,
  output_dir = getOption("fastreg.project_rawdata_dir")
)
...
```{r setup-tree}
#| echo: false
getOption("fastreg.project_rawdata_dir") |>
...
fastreg::convert(
  path = sas_paths[1],
  output_dir = getOption("fastreg.project_workdata_dir")
)
```
...
```{r output-tree-file}
#| echo: false
getOption("fastreg.project_workdata_dir") |>
  # Get to the `E:` directory.
  fs::path_dir() |>
  fs::path_dir() |>
  fs::path_dir() |>
...
```{r use-targets}
pipeline_dir <- fs::path(getOption("fastreg.project_workdata_dir"), "conversion_pipeline")
...
```{r config}
config <- list(
  sas_paths = fastreg::list_sas_files(getOption("fastreg.project_rawdata_dir")),
  output_dir = getOption("fastreg.project_workdata_dir")
)
```
...
#| include: false
fastreg::list_sas_files(getOption("fastreg.project_rawdata_dir")) |>
  purrr::walk(\(path) {
    fastreg::convert(path, getOption("fastreg.project_workdata_dir"))
  })
```
