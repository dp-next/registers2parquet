...
To create the pipeline file, you can use the `use_template()`
function.`use_template()` creates two files: a `_targets.R` pipeline and
a `conversion-log.qmd` Quarto document. By default, when running the
pipeline, the conversion log is written to PDF, named
`conversion-log-<timestamp>.pdf`.
