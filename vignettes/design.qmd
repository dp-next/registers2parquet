...
3.  Read register Parquet files into R as a DuckDB table.
...
| Object | Description |
|------------------------|------------------------------------------------|
| `project_id` | The DST project number to the folder containing the raw or work data. |
| `output_path` | The absolute path to the directory where the Parquet file should be created in. |
| `path` | The absolute path to a file or directory. It can be a single path or multiple paths as a character vector. Is used in functions to list available files or to specify input files. |
| `register_name` | The name of a register used in the file name (without a path), e.g., "bef". It's used in `list` functions to retrieve the paths of objects including that name. |
