...
This is the list of the core functionality in the fastreg
package:
...
```{mermaid}
%%| fig-alt: "A flowchart showing the data flow within fastreg, showing how SAS files are imported, potentially joined, and then exported as Parquet files with a year partitioning."
%%| file: flow.mmd
```
...
fastreg includes two main functions and multiple helper
...
See the `convert_to_parquet()` reference for more details.
...
See the `read_register()` reference for more details.
...
We expect the flow of using the fastreg package to convert
...
fastreg package into an R session to be as follows:
