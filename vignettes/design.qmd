...
3. Provide a [targets](https://docs.ropensci.org/targets/) pipeline
   template to coordinate converting multiple registers in parallel.
4. Provide functions to list available SAS or Parquet register files
   directly from R.

## Interface

We created the interface (the functions and objects that are exposed to
users) based on some specific naming conventions. Specifically, we
generally name function by the **action** they perform and the
**object(s)** they perform it on in the format `{action}_{object}()`.
**Actions** are verbs that describe what a function does, while
**objects** are nouns that represent the objects that the functions
operate on. Below is an overview of the main actions and objects within
fastreg.
