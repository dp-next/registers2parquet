...
- `project_id`: A number indicating the project id, e.g., on Denmark's
  Statistics.
...
flowchart TD
    ...
    %% Edges
    get_project_dir --> list_sas_files --> path --> convert_register
...
fastreg provides three ways to read in a Parquet registers depending on
the use case.

`read_register()` is the main read function. We wanted a function that
could make it really easy to use and read in a particular register
(along with all of it's years if it is in a partition format). For
...
also provide two more flexible functions: `read_parquet_partition()` and
`read_parquet_file()`.

`read_parquet_partition()` underlies `read_register()`, but without
guessing the path. It takes a direct path to the Parquet dataset (the
directory containing the Hive-partitioned Parquet files), applies some
settings to more smoothly read in the datasets, and converts it into a
DuckDB table. This function would be used if `read_register()` failed to
correctly read the right dataset.

`read_parquet_file()` is the simplest read function. It takes a direct
path to a `.parquet` file (not a partitioned dataset) and reads it in as
a DuckDB table. This would be used if the register wasn't in a
partitioned format.

### List SAS and Parquet files

To help with management as well as discovery of available registers
(that we also want to use internally for some of the other functions),
we also provide some helper functions to list the available SAS and
Parquet files and partitioned datasets.

`list_parquet_files()` takes the directory of the project ID and lists
all Parquet files found within either the `rawdata` or `workdata`
directories and following the `part-*.parquet` pattern. We decided to
look in both `rawdata` (where the original SAS files are also kept) as
well as `workdata` because some projects have managers with access to
saving files (like Parquet files) to `rawdata` but other projects don't,
so they need to save files in `workdata`. By default, we also have a
function (`get_project_dir()`) that will automatically find the project
ID directory root from the current working directory. That way, users
can use `list_parquet_files()` without any arguments and it will
automatically find and list all the Parquet files within the project.
...
SAS files found within the `rawdata` directories. We only look in
`rawdata` because DST stores the original SAS files there. Like
`list_parquet_files()`, it also uses `get_project_dir()` by default to
automatically find the project directory and list all the SAS files
within it.
