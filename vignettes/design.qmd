...
- `fastreg.project_rawdata_dir`: The directory where either the SAS or
  Parquet files are stored. The `rawdata/` directory is read-only and
...
- `fastreg.project_workdata_dir`: The `workdata/` directory is where
  ...
  scripts, documents, and other files, but can also store Parquet files.
...
read in the datasets, and converts it into a DuckDB table. This function
would be used if `read_register()` failed to correctly read the right
dataset.
...
are different, the setting must be set. That way, users can use
