...
5. Provide a helper function to get the full path to the project
   directory by its ID.
...
- `project_id`: A double indicating the project id, e.g., on Denmark's
  Statistics.
...
flowchart TD
    identify_paths("Identify register path(s) using get_project_dir() and list_sas_files()")
...
flowchart TD
    identify_path("Identify register path using get_project_dir()")
    path[/"path<br>[Character scalar]"/]
    read_register("read_register()")
    output[/"Output<br>[DuckDB table]"/]

    %% Edges
    identify_path --> path --> read_register --> output

    %% Style
    style identify_path fill:#FFFFFF, color:#000000, stroke-dasharray: 5 5
```
