...
| path | A path to a file or directory. Can be a single path or multiple paths as a character vector. Is used in `list` functions to list available files and in `registers2parquet` to specify input files. |
